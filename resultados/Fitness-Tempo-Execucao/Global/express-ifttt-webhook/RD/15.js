            ...
            return req.on('end', function () {
                parseString(body, { explicitArray: false }, function (err, xml) {
                    if (err || !xml || !xml.methodCall || !xml.methodCall.methodName) {
                        return res.set({ 'Content-Type': 'text/xml' }).status(404).end(util.format(failureXML));
                    }
                    var xmlResponse = '';
                    switch (xml.methodCall.methodName) {
                    case 'mt.supportedMethods':
                        xmlResponse = util.format(successXML);
                        break;
                    ...
                    case 'metaWeblog.newPost':
                        ...
                        function doneCallback(err, results) {
                            ...
                            if (transformed) {
                                ...
                                if (validator.isURL(url)) {
                                    makeRequest(url, lodash.omit(transformed, [
                                        'url',
                                        'categories'
                                    ]));
                                }
                            }
                            return res.status(200).end(util.format(successXML, '<string>' + new Date().getTime() + '</string>'));
                        }
                        var callbacks = lodash(json.categories).map(function (cat) {
                        ...
                        if (authCallback) {
                            ...
                            json = lodash.omit(json, [
                                'username',
                                'password'
                            ]);
                            return authCallback(username, password, function (err, user) {
                                if (err || !user) {
                                    return res.set({ 'Content-Type': 'text/xml' }).status(404).end(util.format(failureXML, 404));
                                }
                        ...
                        var asyncCallbacks = callbacks.map(function (cb) {
                            return async.apply(cb);
                        });
                        return async.parallel(asyncCallbacks, doneCallback);
                    default:
                        break;
                    }
                    return res.set({ 'Content-Type': 'text/xml' }).status(200).end();
                });
            });
        }
        return next();
